...
def rank_and_evaluate(args, subset_to_stats, all_log_probs, labels, metadatas, uids, predictions):
    ...
    for temp, temp_dict in subset_to_stats.items():
        ...
        for chosen_sentence, label, metadata, uid in zip(chosen_sentences, labels, metadatas, uids):
...
def compute_causal_results(args, model, dataloader, temperatures):
    ...
    for sentence_dict, labels, metadatas, uids in tqdm(dataloader):
        ...
        for prefix in prefixes:
            logits = model(
                input_ids=sentence_dict[f"{prefix}_inputs"].cuda(),
                attention_mask=sentence_dict[f"{prefix}_attn_mask"].cuda(),
            )['logits'] # BxTxV

            for temp in subset_to_stats:
                log_probs = F.log_softmax(logits / temp, dim=-1)
                target_log_probs = torch.gather(log_probs, -1, sentence_dict[f"{prefix}_targets"].cuda().unsqueeze(-1)).squeeze(-1)
                phrase_log_probs = torch.sum(target_log_probs * sentence_dict[f"{prefix}_phrase_mask"].cuda(), dim=1)
                all_log_probs[temp].append(phrase_log_probs.cpu())

        rank_and_evaluate(args, subset_to_stats, all_log_probs, labels, metadatas, uids, predictions)

    return subset_to_stats, predictions
    ...
    for sentence_dict, labels, metadatas, uids in tqdm(dataloader):
        ...
        for prefix in prefixes:
            ...
            for batch_idx in range(num_batches):
                # Construct minibatch
                tokens = sentence_dict[f"{prefix}_tokens"][batch_idx*bsz:(batch_idx+1)*bsz].cuda()
                attn_mask = sentence_dict[f"{prefix}_attn_mask"][batch_idx*bsz:(batch_idx+1)*bsz].cuda()
                indices = sentence_dict[f"{prefix}_indices"][batch_idx*bsz:(batch_idx+1)*bsz].cuda()
                targets = sentence_dict[f"{prefix}_targets"][batch_idx*bsz:(batch_idx+1)*bsz].cuda()
                ...
                minibatch_indices = torch.arange(logits.shape[0]).cuda()
