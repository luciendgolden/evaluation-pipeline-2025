...
from dataset_hf import Dataset, collate_function, PredictDataset, predict_collate_function
from classifier_hf import ModelForSequenceClassification
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--config_file", default="classifier_config.json", type=pathlib.Path, help="The local path to the model binary.")
...
if __name__ == "__main__":
    ...
    train_dataloader: DataLoader = DataLoader(train_dataset, batch_size=args.batch_size, collate_fn=partial(collate_function, tokenizer, args.causal, args.sequence_length), shuffle=True, drop_last=True)

    valid_dataloader: DataLoader | None = None
    if args.valid_data is not None:
        valid_dataset: Dataset = Dataset(args.valid_data, args.task)
        valid_dataloader = DataLoader(valid_dataset, batch_size=args.valid_batch_size, collate_fn=partial(collate_function, tokenizer, args.causal, args.sequence_length))
        model_name = args.model_name_or_path.stem
        if args.task == "mnli":
            args.task = args.valid_data.stem.split(".")[0]
        output_path = args.results_dir / model_name / args.task / "finetune"

        if not os.path.exists(args.results_dir):
            os.mkdir(args.results_dir)
        if not os.path.exists(args.results_dir / model_name):
            os.mkdir(args.results_dir / model_name)
        if not os.path.exists(args.results_dir / model_name / args.task):
            os.mkdir(args.results_dir / model_name)
    ...
    if args.predict_data is not None:
        ...
        predict_dataloader = DataLoader(predict_dataset, batch_size=args.valid_batch_size, collate_fn=partial(predict_collate_function, tokenizer, args.causal, args.sequence_length))
    ...
    best_model: nn.Module = train(model, train_dataloader, args, optimizer, scheduler, device, ema_model, valid_dataloader, args.verbose)
