...
def collate_function(tokenizer: Tokenizer, causal: bool, data: list[tuple[str, str] | int]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    if causal:
        attention_mask = attention_mask.unsqueeze(1).repeat(1, attention_mask.size(-1), 1).tril(diagonal=0)
...
def predict_collate_function(tokenizer: Tokenizer, causal: bool, data: list[tuple[str, str] | int]) -> tuple[torch.Tensor, torch.Tensor]:
